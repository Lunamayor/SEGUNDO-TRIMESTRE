...
while seguir == True:
    print("")
    print("=== SISTEMA DE RESERVA DE PELÍCULAS ===")
    print("1.", peliculas[1]["nombre de la pelicula"])
    print("2.", peliculas[2]["nombre de la pelicula"])
    print("3.", peliculas[3]["nombre de la pelicula"])
    print("4.", peliculas[4]["nombre de la pelicula"])
    print("5.", peliculas[5]["nombre de la pelicula"])
    print("6.", peliculas[6]["nombre de la pelicula"])
    print("7.", peliculas[7]["nombre de la pelicula"])
    print("8.", peliculas[8]["nombre de la pelicula"])
    print("0. Salir")

    opcion = input("Elige el número de la película: ")

    if opcion == "0":
        print("Gracias por usar el sistema.")
        seguir = False
    elif opcion in ["1","2","3","4","5","6","7","8"]:
        peli = peliculas[int(opcion)]
        print("")
        print("Has elegido:", peli["nombre de la pelicula"])
        print("Días disponibles:")
        for d in peli["dias disponibles"]:
            print("-", d)

        dia = input("Escribe el día: ").lower().strip()
        clave_horas = "horas de la pelicula " + dia
        if clave_horas in peli:
            print("Horarios disponibles:")
            for h in peli[clave_horas]:
                print("-", h)
            hora = input("Escribe la hora exacta (ej 16:00): ").strip()

            clave_asientos = "asientos disponibles " + dia + " " + hora
            if clave_asientos in peli:
                asientos = peli[clave_asientos]
                if len(asientos) == 0:
                    print("No quedan asientos para esa función.")
                else:
                    print("Asientos disponibles:")
                    for a in asientos:
                        print(a, end=" ")
                    print("")
                    asiento = input("Escribe el número de asiento que quieres: ")
                    if asiento.isdigit():
                        asiento = int(asiento)
                        if asiento in asientos:
                            confirmar = input("¿Confirmas la reserva del asiento " + str(asiento) + "? (s/n): ").lower()
                            if confirmar == "s":
                                asientos.remove(asiento)
                                peli[clave_asientos] = asientos
                                print("Reserva confirmada. Asiento", asiento, "para", peli["nombre de la pelicula"], "el", dia, "a las", hora)
                            else:
                                print("Reserva cancelada.")
                        else:
                            print("Ese asiento ya fue reservado o no existe.")
                    else:
                        print("Debes escribir un número de asiento.")
            else:
                print("No existe esa función a esa hora.")
        else:
            print("Ese día no está disponible.")
    else:
